...
4.1.1. smarty_accounts_login - лог авторизации абонентов
--------------------------------------------------------
...
4.1.2. smarty_billing_out - запросы к внешним системам
------------------------------------------------------
...
4.1.3. smarty_billing_in - входящие запросы к Billing API
---------------------------------------------------------
...
4.1.4. smarty_cache - события, связанные с кешированием
-------------------------------------------------------
...
4.1.5. smarty_messaging - лог отправленных сообщений для аккаунтов
------------------------------------------------------------------
...
4.1.6. smarty_messaging - лог переодических команд
--------------------------------------------------
...
4.1.7. smarty_epg - лог импорта EPG
-----------------------------------
...
Импорт програм завершен, дополнительный контекст:

* ``channels_processed`` - количество обработанных каналов
* ``programms_imported`` - количество импортированных программ


4.1.7. smarty_content_requests - запросы к контенту через api 
-------------------------------------------------------------
...
Успешный запрос к api, дополнительный контекст:

* ``url`` - URL метода, к которому производился запрос
* ``params`` - параметры запроса
* дополнительная инофрмация(в зависимости от метода)


client_channels_not_found
+++++++++++++++++++++++++

В кеше не обнаружены каналы клиента, возможно был сброшен кеш или произошла ошибка выполнения команды ``cache_channel_list``, дополнительный контекст:
