...
* Убедиться, что все миграции установлены: ::

    $ python manage.py migrate --settings=settings.<settings filename>
Если будет предложено выполнить команду ``makemigrations`` - не нужно её делать.
...
* Переустановить пакет обновления smarty, который был установлен в тот момент, когда возникла данная ошибка.
  Временно удалить из папки /tvmiddleware/migrations все новые миграции (датой создания у которых новее чем дата последнего успешного обновления smarty).

* Очистить таблицу ``django_migrations`` в СУБД

* Выполнить команду: ::

    $ python manage.py migrate --fake --settings=settings.<settings filename>

* Скопировать обратно миграции, временно удаленные на шаге 3. Повторно выполнить команду миграции: ::
