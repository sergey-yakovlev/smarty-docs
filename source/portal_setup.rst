...
show_password_recovery ``bool``
    Флаг, который включает возможность восстановления пароля через отправку СМС. 
    По умолчанию: false. Поддерживается только в шаблонах: ``futuristic``.

reserve_api_url ``str``
    Адрес резервного сервера Smarty.
    По умолчанию: *не задано*.
    Поддерживается только в шаблонах: ``futuristic``.

health_check_interval ``int``
    Временной интервал, через который будут делаться запросы на проверку состояния сервера. Интервал задается в миллисекундах.
    По умолчанию: *300000*.
    Поддерживается только в шаблонах: ``futuristic``.

health_check_attemts ``array``
    Задает количество попыток соединения с сервером Smarty и временные промежутки между попытками в случае потери соединения с текущим сервером Smarty. Задается в виде массива. Каждый элемент массива - значение в секундах, через которое будет делаться повторный запрос к текущему серверу Smarty. Если ни одна из попыток не была успешной, происходит переключение на резервный сервер Smarty.
    По умолчанию: *[30]*.
    Поддерживается только в шаблонах: ``futuristic``.
