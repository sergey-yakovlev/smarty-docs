...
show_offer_accept ``bool``
    Флаг, отвечающий за показ попапа с подтверждением согласия с офертой перед оплатой.
    По умолчанию *false*.
    Поддерживается только в шаблонах: ``impuls``.

autohide_timer ``number``
    Тайм-аут (в секундах), по истечению которого должен быть скрыт текущий экран и открыт экран плеера, если в настоящий
